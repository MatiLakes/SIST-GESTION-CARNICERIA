...
// Esquema de validación para crear y actualizar un repartidor
const repartidorSchema = Joi.object({
  nombre: Joi.string()
    .min(2)
    .max(255)
    .required()
...
// Exportar las validaciones para el cuerpo de la solicitud de repartidor
export const repartidorBodyValidation = {
  validate: (data) => repartidorSchema.validate(data),
};

// Exportar las validaciones para actualizar el repartidor
export const repartidorUpdateBodyValidation = {
  validate: (data) => repartidorSchema.validate(data),
};

// Esquema de validación para la consulta de repartidor
export const repartidorQueryValidation = Joi.object({
  id: Joi.number()
    .integer()
    .positive()
    .required()
