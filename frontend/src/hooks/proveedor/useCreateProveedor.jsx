import { createProveedorService } from '@services/proveedor.service.js'; // Asegúrate de que el servicio esté correctamente importado
...
const useCreateProveedor = (fetchProveedores) => {
    const handleCreate = async (proveedorData) => {
        try {
            const newProveedor = await createProveedorService(proveedorData); // Llamada al servicio para crear el proveedor
            if (newProveedor) {
